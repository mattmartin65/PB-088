...
var JotForm = {
    ...
    */
    ...
    conditionDebugger: function() {
        ...
        const resolve = (affectedFields = [], baseField) => {
            ...
            if (isLooped) {
                ...
                const path = calculationLoops.get(baseField);
                calculationLoops.set(baseField, [...path, ...affectedFields]);
                return;
            }
            affectedFields.forEach(affectedField => {
                const relatedAffectedFields = calculationMap.get(affectedField);
                if (!relatedAffectedFields || relatedAffectedFields.has(affectedField)) return;
    ...
    handleIFrameHeight: function () {
        ...
        var height = Math.max(form.clientHeight, form.scrollHeight, form.offsetHeight)
        var isEmbedForm = document.querySelectorAll(".supernova.isEmbeded").length > 0;

        if (isEmbedForm && form.offsetWidth === 0) {
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    checkCondition: function (condition, sourceField, sourceEvent) {
        ...
        const ignoreFieldConditionLoopCheck = Boolean((operands || []).find(operand => ignoreFields.includes(JotForm.getInputType(operand))));
                ...
                setTimeout(function (calc) {
                    ...
                    if (!matchForThisResult[condition.action[0].resultField+subfield]) {
                        try {
                            ...
                            if (!JotForm.isEditMode() && !JotForm.draftID) {
    ...
    checkCalculation: function (calc, sourceField, sourceEvent) {
        ...
        const ignoreFieldCalculationLoopCheck = Boolean((operands || []).find(operand => ignoreFields.includes(JotForm.getInputType(operand))));
   ...
   */
    ...
    isValidMinTotalOrderAmount: function () {
        if (JotForm.hasMinTotalOrderAmount() && parseInt(JotForm.minTotalOrderAmount) > 0) {
            ...
            if (!!(JotForm.pricingInformations.general.discount > 0)) {
